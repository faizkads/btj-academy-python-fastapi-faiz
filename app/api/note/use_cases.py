...
class CreateNote:
    ...
    async def execute(self, user_id: int, request: CreateNoteRequest) -> NoteSchema:
        async with self.async_session.begin() as session:
            ...
            existing_note = existing_note.scalars().first()

            if existing_note:
                raise HTTPException(
                    400, f"Title: {request.title} is already used by another note"
                )
...
class UpdateNote:
    def __init__(self, session: AsyncSession) -> None:
        self.async_session = session
    async def execute(self, user_id: int, note_id: int, request: UpdateNoteRequest) -> NoteSchema:
        async with self.async_session.begin() as session:
            note = await session.execute(
                select(Note).where(
                (Note.created_by == user_id) & (Note.note_id == note_id) & (Note.deleted_at == None)
